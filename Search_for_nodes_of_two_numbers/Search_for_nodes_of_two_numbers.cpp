...
int main()
{
    ...
    if (num1 < num2)
    {
        printf("Числитель \"%s\" меньше, чем знаменатель \"%s\"\n", number1, number2);
        dataCorrect = false;
    }
    if (num3 < num4)
    {
    ...
    if (!dataCorrect) //Если данные некорректны (числитель меньше знаменателя) - завершить программу
        return 1;

    //Проверка чисел на допустимый диапазон [1..3000]
    if (num1 < 1 || num1 > 3000)
    {
        printf("Число \"%s\" не принадлежит диапазону[I..MMM].\n", number1);
        dataCorrect = false;
    }
    if (num2 < 1 || num2 > 3000)
    {
    ...
    if (!dataCorrect) //Если данные некорректны (числа не принадлежат диапазону) - завершить программу
        return 1;
